...
- `model` and `fallbackModels` accept a `provider/modelId:thinking` suffix (`minimal` / `low` / `medium` / `high` / `xhigh` / `max`). Naming calls now go through `completeSimple` so the suffix maps to Pi's unified `reasoning` option.
